...
def get_all_sentences(ds):
    for item in ds:
        yield item['query_checker']['query']
...
def get_ds(config):
    ds_raw = load_dataset('Mostafijur/medical_and_non_medical_query_data', split='train')
    ...
    max_len_src = 0
    for item in ds_raw:
        src_ids = tokenizer.encode(item['query_checker'][config['input_src']]).ids
